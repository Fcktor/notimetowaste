    ...
    <div className="min-h-screen flex" style={{ background: "#FBFBFA" }}>
      ...
      >
        <div className="w-full max-w-sm">
          ...
          {step === "form" ? (
            <>
              <h2 className="font-display italic mb-2" style={{ fontSize: "2rem", color: "#2F3437", fontWeight: 400 }}>
                Crear cuenta
              </h2>
              ...
              <div className="mt-10 pt-6" style={{ borderTop: "1px solid #EAEAEA" }}>
                <p className="text-xs" style={{ color: "#787774" }}>
                  ¿Ya tienes cuenta?{" "}
                  <Link href="/login" style={{ color: "#111111" }}>Inicia sesión</Link>
                </p>
              </div>
            </>
          ) : (
            <>
              ...
              <p className="text-sm mb-1" style={{ color: "#787774" }}>
                Enviamos un código de 6 dígitos a
              </p>
              <p className="text-sm font-semibold mb-8" style={{ color: "#111111" }}>{form.email}</p>
              ...
              <form onSubmit={handleVerify} className="space-y-4">
                ...
                <button type="submit" disabled={loading || code.length < 6}
                  className="w-full py-3 text-xs uppercase tracking-[0.14em] transition-all duration-200 disabled:opacity-50 rounded-md bg-[#111111] text-white hover:bg-[#333333] active:scale-[0.98]"
                  style={{ fontFamily: "var(--font-geist-sans)" }}>
              ...
              <div className="flex items-center justify-between mt-6">
                <button onClick={() => { setStep("form"); setError("") }}
                  className="text-xs transition-colors" style={{ color: "#787774" }}>
                  ← Cambiar correo
                </button>
                <button onClick={handleResend} disabled={loading}
                  className="text-xs transition-colors disabled:opacity-50" style={{ color: "#111111" }}>
                  Reenviar código
                </button>
              </div>
            </>
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden flex-col items-center justify-center gap-10" style={{ background: "#F1F0ED" }}>
        ...
        <div className="relative z-10 text-center">
          <h1 className="font-display italic leading-none" style={{ fontSize: "2rem", color: "#787774", fontWeight: 400, letterSpacing: "0.04em" }}>
            No Time To Waste
          </h1>
