  ...
  return (
    <div className="store-card flex flex-col group">
      ...
      <Link href={`/products/${encodeURIComponent(product.id)}`}>
        <div className="relative overflow-hidden" style={{ aspectRatio: "1/1", background: "#F1F0ED" }}>
          {product.image_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              <svg width="36" height="36" viewBox="0 0 32 32" fill="none" style={{ opacity: 0.3 }}>
          ...
          >
            <span className="text-[10px] uppercase tracking-[0.18em]" style={{ color: "#FFFFFF" }}>
              Ver detalles
            </span>
          </div>
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {discount && (
              <span
                ...
                style={{ background: "rgba(0,0,0,0.75)", color: "#FFFFFF", borderRadius: "2px" }}
              >
                −{discount}%
              </span>
            )}
            {product.stock > 0 && product.stock <= 3 && (
              <span
                className="text-[9px] uppercase tracking-widest px-2 py-0.5"
                style={{ background: "#FDEBEC", color: "#9F2F2D", border: "1px solid #EAEAEA", borderRadius: "2px" }}
              >
          ...
          {product.condition && inStock && (
            <div className="absolute top-3 right-3">
              <span
                ...
                style={{
                  background: "rgba(0,0,0,0.7)",
                  color: product.condition === "Vintage" ? "#FFFFFF" : "rgba(255,255,255,0.7)",
                  border: `1px solid ${product.condition === "Vintage" ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.15)"}`,
                  borderRadius: "2px",
                }}
              >
                {product.condition}
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        <div>
          <h3
            ...
            style={{ color: "#111111", fontWeight: 600, fontStyle: "italic" }}
          >
            {product.model}
          </h3>
          {product.movement && (
            <p className="text-[10px] mt-0.5" style={{ color: "#787774" }}>{product.movement}</p>
          )}
        </div>

        {/* Price */}
        <div className="mt-auto pt-2" style={{ borderTop: "1px solid #EAEAEA" }}>
          <div className="flex items-baseline gap-2 mb-3">
            <span
              className="text-2xl font-semibold tabular-nums"
              style={{ color: "#111111", fontFamily: "var(--font-geist-sans)" }}
            >
              S/ {product.price.toLocaleString("es-PE")}
            </span>
            {product.compare_at_price && product.compare_at_price > product.price && (
              <span className="text-xs line-through tabular-nums" style={{ color: "#787774" }}>
          ...
          {inStock ? (
            <a
              ...
              className="flex items-center justify-center gap-2 w-full py-2.5 text-xs uppercase tracking-[0.14em] transition-all duration-200 active:scale-[0.98]"
              style={{
                color: "#FFFFFF",
                background: "#111111",
